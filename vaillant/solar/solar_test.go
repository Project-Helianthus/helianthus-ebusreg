...
func TestProviderMethods(t *testing.T) {
	...
	plane := planes[0]
	if plane.Name() != "solar" {
		t.Fatalf("expected plane name solar, got %s", plane.Name())
	}
	...
	if methods[1].Name() != methodGetSolarYield || !methods[1].ReadOnly() {
		t.Fatalf("unexpected get_solar_yield method definition")
	}
	...
	if methods[1].Template().Primary() != 0xB5 || methods[1].Template().Secondary() != 0x04 {
		t.Fatalf("unexpected get_solar_yield template")
	}
