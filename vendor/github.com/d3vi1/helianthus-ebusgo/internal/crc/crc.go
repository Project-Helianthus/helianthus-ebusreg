package crc

// CRC8 lookup table for polynomial 0x9b = x^8 + x^7 + x^4 + x^3 + x^1 + 1.
var lookupTable = [256]byte{
	,
}

// Update returns the CRC8 updated with the provided value.
func Update(crc, value byte) byte {
	return lookupTable[crc] ^ value
}

// Sum returns the CRC8 for the provided byte slice.
func Sum(data []byte) byte {
	crc := byte(0)
	for _, b := range data {
		crc = Update(crc, b)
	}
	return crc
}
